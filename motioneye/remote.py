...
def set_motion_detection(local_config, status):
    scheme, host, port, username, password, path, _ = _remote_params(local_config)

    logging.debug('setting motion detection to %(status)s for %(url)s' % {
                  'url': pretty_camera_url(local_config, camera=False), 'status': status})

    request = _make_request(scheme, host, port, username, password,
                            path + '/detection/%s/' % status)

    def on_response(response):
        if response.error:
            logging.error('failed to set motion detection for remote cameras on %(url)s: %(msg)s' % {
